                        ...
                        <div class="bg-gray-50 px-6 py-4 border-b flex justify-between items-center">
                            ...
                            <div class="flex items-center gap-3">
                                <a href="{{ route('orders.invoice', $order) }}"
                                    class="bg-gray-700 text-white px-4 py-2 rounded hover:bg-gray-800 font-semibold">
                                    Download Invoice
                                </a>
                                ...
                                @foreach($order->orderItems as $item)
                                    <div class="flex items-center justify-between border-b pb-3">
                                        <div class="flex items-center space-x-4">
                                            <img src="{{ $item->book->cover_image ? asset('storage/' . $item->book->cover_image) : '/images/no-cover.jpg' }}"
                                                alt="{{ $item->book->title }}" class="w-12 h-16 object-cover rounded">
                                            <div>
        ...
        @else
            <div class="text-center py-12 bg-white shadow-md rounded-lg">
                ...
                <a href="{{ route('dashboard') }}"
                    class="mt-4 inline-block bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">Start Shopping</a>
            </div>
        @endif
    </div>
</x-app-layout>